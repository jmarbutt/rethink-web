...
Click **Run**. You should see a JSON result with `todoId` and `alreadyCompleted: false`. Refresh the task edit page in your browser at `http://localhost:5099/tasks/<id>` to confirm `Completed` is now checked and `CompletedAt` was auto-stamped by the `EntitySaved<Todo>` subscriber. **Same data path as the HTML form post — same auth, same audit, same subscribers.**
...
This is what `tests/RethinkWeb.Sample.Tasks.Tests/EndToEndTests.cs` does. Useful for integration tests or building your own client.
...
For tests against `WebApplicationFactory<Program>`, the test in this repo uses an **in-memory pipe transport** (no real HTTP) — see `MCP_tools_call_invokes_mark_complete_via_real_McpClient` for the pattern.
